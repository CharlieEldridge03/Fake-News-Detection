"""
Portfolio_Ensemble_SHAP.py: Creates or Loads Ensemble Models then produces SHAP plots for the selected ensemble model.
...
def create_ensemble(X_train, y_train, linear_svc, rbf_svc, sigmoid_svc, ensemble_type, final_model, ensemble_file_name):
    ...
    # specified file name.
        ...
        if ensemble_type == "stacking":
            print("Constructing a Stacking Classifier.")
            if final_model == "linear":
                print("Final Stacking Classifier Model is Linear.\n")
                ensemble_svc = StackingClassifier(
                    estimators=[('sigmoid_svc', sigmoid_svc), ('rbf_svc', rbf_svc)], final_estimator=linear_svc,
                    verbose=1, n_jobs=-1)
            elif final_model == "rbf":
                print("Final Stacking Classifier Model is RBF.\n")
                ensemble_svc = StackingClassifier(
                    estimators=[('sigmoid_svc', sigmoid_svc), ('linear_svc', linear_svc)], final_estimator=rbf_svc,
                    verbose=1, n_jobs=-1)
            elif final_model == "sigmoid":
                print("Final Stacking Classifier Model is Sigmoid.\n")
                ensemble_svc = StackingClassifier(
                    estimators=[('rbf_svc', rbf_svc), ('linear_svc', linear_svc)], final_estimator=sigmoid_svc,
                    verbose=1, n_jobs=-1)
            else:
                print("Final Stacking Classifier Model is a Logistic Regressor.\n")
                ensemble_svc = StackingClassifier(
                    estimators=[('sigmoid_svc', sigmoid_svc), ('rbf_svc', rbf_svc), ('linear_svc', linear_svc)],
                    verbose=1, n_jobs=-1)

        else:
            print("Constructing a Voting Classifier.\n")
            ensemble_svc = VotingClassifier(
                estimators=[('sigmoid_svc', sigmoid_svc), ('rbf_svc', rbf_svc), ('linear_svc', linear_svc)],
                verbose=1, n_jobs=-1)
...
def display_summary_plots(shap_values, X_test, feature_names, dataset_name, ensemble_file_name):
    ...
    plt.savefig("{}_summary_bar_plot.png".format(ensemble_file_name))
    plt.close()
    ...
    for i in range(0, num_classes):
        ...
        plt.savefig("{}_class{}_bar_plot.png".format(ensemble_file_name, i))
        plt.close()
        ...
        plt.savefig("{}_class{}_scatter_plot.png".format(ensemble_file_name, i))
        plt.close()


def perform_experiment(dataset_name, feature_set_name, ensemble_type, final_model):
    # This will be used as the base file name for any ensemble model files and confusion matrices produced.
    ensemble_file_name = "{}_{}_{}Ensemble_{}_Final".format(dataset_name, feature_set_name, ensemble_type.title(),
                                                            final_model)
    ...
    ensemble = create_ensemble(X_train, y_train, linear_svc, rbf_svc, sigmoid_svc, ensemble_type, final_model,
                               ensemble_file_name)
    ...
    if feature_set_name == "tfidf":
        if dataset_name == "liar_dataset":
            X_test = pd.DataFrame(X_test.toarray(), columns=feature_names).sample(n=100, random_state=42)
            background_data = pd.DataFrame(X_train.toarray(), columns=feature_names).sample(n=50, random_state=42)

        else:
            print("Defaulting to 10 test samples and 10 training background samples.")
        ...
        X_test = pd.DataFrame(X_test.toarray(), columns=feature_names).sample(n=50, random_state=42)
        background_data = pd.DataFrame(X_train.toarray(), columns=feature_names).sample(n=25, random_state=42)
...
def main():
    ...
    # Specifying which type of ensemble to use [Options are: "voting", "stacking"]
    ensemble_type = "stacking"
    # Specifying the final model to be used with the stacking classifier [Options are: "linear", "rbf", "sigmoid", "none"]
    # When "none" is specified a logistic regressor is constructed as the final model.
    final_model = "none"

    perform_experiment(dataset_name, feature_set_name, ensemble_type, final_model)
