...
def get_cleaned_tokens(text, lemmatizer):
    ...
    # Tokenize text.
    full_text_tokens = [word for word in word_tokenize(text)]

    # Filter tokens to remove stopwords.
    non_stopword_tokens = [token.lower() for token in full_text_tokens if token not in stop_words]
...
def process_ISOT_dataset():
    ...
    ISOT_data = {"text": [], "conjunction_count": [], "pronoun_count": [], "adjective_count": [], "adverb_count": [],
    ...
    for file_path in file_paths:
        ...
        # Change this back to .text to use the article texts.
        file_text = file_contents.title
        ...
        for text in file_text:
            ...
            ISOT_data["text"].append(lemmatized_text)
            ISOT_data["conjunction_count"].append(conjunction_count)
            ISOT_data["pronoun_count"].append(pronoun_count)
            ISOT_data["adjective_count"].append(adjective_count)
            ISOT_data["verb_count"].append(verb_count)
            ISOT_data["noun_count"].append(noun_count)
            ISOT_data["adverb_count"].append(adverb_count)
            ISOT_data["flesch_reading_ease"].append(flesch_reading_ease)
            ISOT_data["flesch_kincaid_grade_level"].append(flesch_kincaid_grade_level)
            ISOT_data["gunning_fog_scale"].append(gunning_fog_scale)
            ISOT_data["coleman_liau_index"].append(coleman_liau_index)
            ISOT_data["linsear_write_formula"].append(linsear_write_formula)
            ISOT_data["dale_chall_readability"].append(dale_chall_readability)
            ISOT_data["mcalpine_eflaw_readability"].append(mcalpine_eflaw_readability)
            ISOT_data["reading_time"].append(reading_time)
            ISOT_data["syllable_count"].append(syllable_count)
            ISOT_data["lexicon_count"].append(lexicon_count)
            ISOT_data["polysyllable_count"].append(polysyllable_count)
            ISOT_data["monosyllable_count"].append(monosyllable_count)
            ISOT_data["stopword_count"].append(stopword_count)
            ISOT_data["present_tense_count"].append(present_tense_count)
            ISOT_data["past_tense_count"].append(past_tense_count)
            ISOT_data["label"].append(label)
...
def main():
    ...
    processed_ISOT_data = process_ISOT_dataset()
    ...
    # -------------------------
    ISOT_dataframe = pd.DataFrame(processed_ISOT_data, columns=column_names)
    LIAR_dataframe = pd.DataFrame(processed_LIAR_data, columns=column_names)

    ISOT_dataframe = normalise_data(ISOT_dataframe)
    LIAR_dataframe = normalise_data(LIAR_dataframe)

    ISOT_dataframe.to_csv("{}\\processed_ISOT.csv".format(ISOT_output_directory), index=False)
