...
def create_tfidf_features(X_train, X_test, ngram_range):
    # tfidf = TfidfVectorizer(ngram_range=ngram_range, min_df=1)
...
def main():
    ...
    # Uncomment to use TFIDF features.
    X_train, X_test = create_tfidf_features(X_train, X_test, ngram_range)

    # Uncomment to use linguistic features.
    # X_train, X_test = get_linguistic_features(X_train, X_test)

    # Uncomment to random-undersample the dataset classes to the level of the least-featured class.
    # X_train, y_train = undersample(X_train, y_train)

    # Uncomment to SMOTE oversample the dataset classes to the level of the most-featured class.
    X_train, y_train = oversample(X_train, y_train)

    # Uncomment to use single value decomposition on the TFIDF feature set (intended for use with the ISOT dataset)
