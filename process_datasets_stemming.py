...
def get_cleaned_tokens(text):
    ...
    # Stem remaining non-stopword tokens.
    stemmed_tokens = [stemmer.stem(token) for token in non_stopword_tokens]

    return full_text_tokens, stemmed_tokens
...
def process_ISOT_dataset():
    ...
    for file_path in file_paths:
        ...
        for _, row in file_contents.iterrows():
            ...
            full_title_tokens, stemmed_tokens_title = get_cleaned_tokens(title)
            full_text_tokens, stemmed_tokens_text = get_cleaned_tokens(text)

            stemmed_tokens_title = " ".join(stemmed_tokens_title)
            stemmed_tokens_text = " ".join(stemmed_tokens_text)
            ...
            ISOT_title_data["text"].append(stemmed_tokens_title)
            ...
            ISOT_text_data["text"].append(stemmed_tokens_text)
...
def process_LIAR_dataset():
    ...
    for file_path in file_paths:
        ...
        for _, row in file_contents.iterrows():
            ...
            full_text_tokens, stemmed_tokens = get_cleaned_tokens(text)
            ...
            LIAR_data["text"].append(stemmed_tokens)
