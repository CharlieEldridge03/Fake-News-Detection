...
def load_isot_data():
    ...
    ISOT_dataframe[["real", "fake"]] = pd.DataFrame(ISOT_dataframe["label"].map(label_dict).tolist(),
                                                    index=ISOT_dataframe["label"].index)
...
def main():
    ...
    trainer.train()
    trainer.evaluate()
    trainer.save_model("bert-fake-news-finetuned")
